...
def chamfer_loss(point_cloud1, point_cloud2):
    ...
    min_dist_a_to_b, _ = torch.min(dist_a_to_b, dim=-1)  # 对每个点取最小距离
    loss_a_to_b = torch.mean(min_dist_a_to_b, dim=-1)    # 均值

    # 计算反向距离（B -> A）
    dist_b_to_a = torch.cdist(point_cloud2, point_cloud1, p=2.0)
    min_dist_b_to_a, _ = torch.min(dist_b_to_a, dim=-1)
    loss_b_to_a = torch.mean(min_dist_b_to_a, dim=-1)
